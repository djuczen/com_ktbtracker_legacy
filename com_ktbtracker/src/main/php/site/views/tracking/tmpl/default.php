...
HTMLHelper::addIncludePath(JPATH_BASE . '/components/com_ktbtracker/helpers/html');
...
$cycle_start = HTMLHelper::_('date', $this->cycle->cycle_start, 'F j, Y', null);
$cycle_finish = HTMLHelper::_('date', $this->cycle->cycle_finish, 'F j, Y', null);
$week_start = HTMLHelper::_('date', $this->items[0]->tracking_date, 'F j, Y', null);
$week_finish = HTMLHelper::_('date', $this->items[6]->tracking_date, 'F j, Y', null);
...
<div class="jsn-p">
	<div class="jsn-p-top jsn-p-top-a">
		...
		<div class="jsn-p-title">
			...
			<div data-toggle="tooltip" title="<?php echo ($this->candidate->online) ? Text::_('COM_KTBTRACKER_STATUS_ONLINE') : Text::_('COM_KTBTRACKER_STATUS_OFFLINE'); ?>"  class="status label label-<?php echo ($this->candidate->online) ? 'success' : 'danger'; ?>"></div>
		</div>
		<div class="jsn-p-before-fields">
			<div class="jsn-p-dates">
				<div class="jsn-p-date-reg">
					<b><i class="fa fa-align-justify"></i> <?php echo Text::_('COM_KTBTRACKER_FIELD_TRACT_LABEL'); ?>: </b>
		...
		<div class="jsn-p-fields">
			<fieldset class="jsn-form-fieldset" id="jsn_default" data-index="0" data-name="details">
				<dl class="dl-horizontal">
					<dt><?php echo Text::_('COM_KTBTRACKER_FIELD_ABOUTME_LABEL'); ?></dt>
					<dd>
						<?php echo array_key_exists('aboutme', $this->candidate->profile->profile) ? $this->escape($this->candidate->profile->profile['aboutme']) : Text::_('COM_KTBTRACKER_NO_INFORMATION_PROVIDED'); ?>
					</dd>
				</dl>
			</fieldset>
		</div>
	...
	<div class="table-responsive">
		<table class="tracking-table table table-bordered table-condensed">
			<thead>
				<tr>
					<th class="tracking-header text-center" colspan="12">
						...
						<h4>&nbsp;&nbsp;<?php echo Text::sprintf('COM_KTBTRACKER_HEADING_CYCLE_TITLE', $this->escape($this->cycle->title), $cycle_start, $cycle_finish); ?>&nbsp;&nbsp;</h4>
						<h3><?php echo Text::sprintf('COM_KTBTRACKER_HEADING_CYCLE_WEEK', $cycleweek->week, $week_start, $week_finish); ?></h3>
					</th>
				</tr>
				<tr>
					<th width="25%"><?php echo Text::_('COM_KTBTRACKER_HEADING_REQUIREMENT'); ?></th>
					...
					<th style="text-align: center;" width="<?php echo round(60/7); ?>%">
						...
						<a href="<?php echo $item->editLink; ?>" class="btn btn-default btn-xs hasTip" title="Edit::<?php echo $item->tracking_date; ?>">
							<i class="fa fa-pencil"></i>&nbsp;&nbsp;<?php echo Text::_('COM_KTBTRACKER_BUTTON_EDIT_LABEL'); ?></a><br />
						<?php endif; ?>							
						<span class="hasTip" title="<?php echo $item->tracking_day; ?>::<?php echo $item->tracking_date; ?>"><?php echo $item->tracking_day; ?><br />
							<?php echo HTMLHelper::_('date', $item->tracking_date, 'm-d', null); ?></span>
					</th>
					<?php endforeach; ?>
					<th width="3%"><?php echo Text::_('COM_KTBTRACKER_HEADING_TOTAL'); ?></th>
					<th width="9%"><?php echo Text::_('COM_KTBTRACKER_HEADING_PROGRESS'); ?></th>
					<th width="3%"><?php echo Text::_('COM_KTBTRACKER_HEADING_GOAL'); ?></th>
					<th width="3%"><?php echo Text::_('COM_KTBTRACKER_HEADING_REMAINING'); ?></th>
				</tr>
			</thead>
			<tbody>
				<?php 
				    $label      = Text::_('COM_KTBTRACKER_FIELD_JOURNALS_LABEL');
				    $title      = $this->escape(Text::_('COM_KTBTRACKER_FIELD_JOURNALS_DESC'));
				...
				<?php endforeach; ?>
					...
					<td class="text-right" width="3%">
						<?php echo HTMLHelper::_('ktbformat.number', $progress->goal); ?><br/>
					</td>
					<td class="text-right" width="3%">
						<?php echo HTMLHelper::_('ktbformat.number', $progress->remaining); ?><br/>
					</td>
				</tr>
			<?php foreach (KTBTrackerHelper::getCycleRequirements(true) as $reqmnt) : ?>
				...
				<?php 
				    $label      = Text::_('COM_KTBTRACKER_FIELD_' . $reqmnt . '_LABEL');
				    $title      = $this->escape(Text::_('COM_KTBTRACKER_FIELD_' . $reqmnt . '_DESC'));
				...
				<?php foreach ($this->items as $d => $item) : ?>
					...
					<td style="text-align: right;">
						<?php echo HTMLHelper::_('ktbformat.number', $item->$reqmnt); ?>
					</td>
					<?php endif; ?>
				<?php endforeach; ?>
					<?php
